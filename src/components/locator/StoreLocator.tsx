...
export default function StoreLocator({
  ...
  const filtered = useMemo(() => {
    ...
    return byType
      ...
      .map((l) => {
        ...
        return { ...l, distanceKm: d } as LocatorLocation & {
          distanceKm: number;
        };
      })
  ...
  return (
    <div className="space-section">
      <div
        className={
          edgeToEdge ? "max-w-7xl mx-auto px-0" : "max-w-6xl mx-auto px-6"
        }
      >
        <h1
          className="text-4xl lg:text-5xl font-light text-[#2C2B29] mb-2"
          style={{ fontFamily: "Playfair Display, serif" }}
        >
          Find your closest outlet
        </h1>
        <p className="text-[#7A7771] mb-6">
          Search by area, use your current location
          {compact ? "" : ", then filter by distance and type"}.
        </p>
        ...
        >
          ...
          <div
            className={`col-span-1 ${
              compact ? "lg:col-span-12" : "lg:col-span-5"
            }`}
          >
            <div className="card rounded-2xl border border-[#E8E2DC] bg-white/80 shadow-md p-4 sm:p-6">
              ...
              <div className="flex gap-2 mb-4">
                ...
                {compact && (
                  <Link href="/outlets" className="btn btn-outline">
                    Open full map
                  </Link>
                )}
              </div>
              ...
              {!compact && (
                <div className="mb-4">
                  <label className="text-sm text-[#7A7771]">
                    Radius: {radiusKm} km
                  </label>
                  <input
                    type="range"
              ...
              <div className="border-t border-[#E8E2DC] pt-3 max-h-[58vh] overflow-auto">
                {filtered.length === 0 ? (
                  <div className="text-[#7A7771]">
                    No locations found within {radiusKm} km.
                  </div>
                ) : (
                  <ul className="space-y-3">
                    {filtered.map((l) => (
                      <li
                        key={l.id}
                        className="p-3 rounded-lg border border-[#E8E2DC] bg-white/80"
                      >
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <div className="text-sm text-[#7A7771] whitespace-nowrap">
                            {(l as any).distanceKm.toFixed(1)} km
                          </div>
                        </div>
          ...
          {!compact && (
            <div className="lg:col-span-7 lg:sticky lg:top-24">
              <DynamicLeafletMap
                ...
                height="65vh"
              />
              <div className="text-xs text-[#7A7771] mt-2">
