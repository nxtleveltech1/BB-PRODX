/**
 * Reset Database Sequences and Import Products
 *
 * This script:
 * 1. Resets ID sequences for all tables
 * 2. Creates the product_images table
 * 3. Imports all 37 products from hardcoded data
 *
 * Usage: tsx scripts/reset-sequences-and-import.ts
 */
...
async function resetSequences() {
  ...
  try {
    ...
    `);
    console.log('  ✓ Categories sequence reset');

    await db.execute(sql`
      SELECT setval('subcategories_id_seq', COALESCE((SELECT MAX(id) FROM subcategories), 1), true);
    `);
    console.log('  ✓ Subcategories sequence reset');

    await db.execute(sql`
      SELECT setval('products_id_seq', COALESCE((SELECT MAX(id) FROM products), 1), true);
    `);
    console.log('  ✓ Products sequence reset');

    await db.execute(sql`
      SELECT setval('product_benefits_id_seq', COALESCE((SELECT MAX(id) FROM product_benefits), 1), true);
    `);
    console.log('  ✓ Product benefits sequence reset');

    await db.execute(sql`
      SELECT setval('product_ingredients_id_seq', COALESCE((SELECT MAX(id) FROM product_ingredients), 1), true);
    `);
    console.log('  ✓ Product ingredients sequence reset');

    await db.execute(sql`
      SELECT setval('product_tags_id_seq', COALESCE((SELECT MAX(id) FROM product_tags), 1), true);
    `);
    console.log('  ✓ Product tags sequence reset');
    ...
    if (category.subcategories) {
      for (const subcategory of category.subcategories) {
        const [insertedSubcategory] = await db
          ...
          .onConflictDoUpdate({
            target: schema.subcategories.slug,
            set: {
              categoryId: insertedCategory.id,
              name: subcategory.name,
              description: subcategory.description,
...
// Main execution
async function main() {
  console.log('🚀 Starting Complete Database Import Process');
  console.log(`📊 Products to import: ${sourceProducts.length}`);

  try {
    // Step 0: Reset sequences
    const sequencesReset = await resetSequences();
    if (!sequencesReset) {
      console.warn('⚠️  Failed to reset sequences, continuing anyway...');
    }

    // Step 1: Create product_images table
    const tableCreated = await createProductImagesTable();
    if (!tableCreated) {
      console.warn('⚠️  Failed to create product_images table, may already exist');
    }
